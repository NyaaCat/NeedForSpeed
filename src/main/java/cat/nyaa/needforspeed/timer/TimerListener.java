...
public class TimerListener implements Listener {
    ...
    public void playerMove(PlayerMoveEvent e) {
        ...
        if (plugin.getTimerConfig().getTimerCheckInterval() != -1) {
            if (lastCheck.containsKey(p.getUniqueId()) &&
                    !(System.currentTimeMillis() - lastCheck.get(p.getUniqueId()) >= plugin.getTimerConfig().getTimerCheckInterval())) {
                return;
            } else {
                lastCheck.put(p.getUniqueId(), System.currentTimeMillis());
            }
        }
        ArrayList<Checkpoint> checkpointList = plugin.getTimerManager().getCheckpoint(p, true);
        if (!checkpointList.isEmpty()) {
            for (Checkpoint checkpoint : checkpointList) {
                Timer timer = plugin.getTimerManager().getTimer(checkpoint.getTimerName());
